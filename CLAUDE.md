...
### 9. Frontend: React SPA conventions
- **Components** live in `frontend/src/components/`, grouped by feature
- **API hooks** are auto-generated by orval (`npx orval`) — never write fetch calls manually
- **Encryption** is handled at the TanStack Query layer (`use-crypto.ts`) — components receive plaintext
- **UI primitives**: use shadcn/ui components as building blocks
- **Styling**: Tailwind CSS v4 — no standalone CSS files
