...
async def materialize_all_instances() -> dict[str, Any]:
    ...
    Uses per-user session scope to prevent ORM object accumulation.
...
async def run_materialization_loop() -> None:
    ...
    Includes timeout protection to prevent runaway materialization cycles.
    """
    first_run = True
    while True:
        try:
            ...
            # Materialize new instances + clean up old ones (with timeout)
            async def _run_cycle() -> tuple[dict[str, Any], dict[str, Any]]:
                stats = await materialize_all_instances()
                cleanup = await cleanup_old_instances()
                return stats, cleanup

            stats, cleanup = await asyncio.wait_for(_run_cycle(), timeout=MATERIALIZATION_TIMEOUT_SECONDS)
            ...
            logger.error(
                f"Materialization cycle timed out after {MATERIALIZATION_TIMEOUT_SECONDS}s - will retry next cycle"
            )
            if first_run:
                first_run = False
