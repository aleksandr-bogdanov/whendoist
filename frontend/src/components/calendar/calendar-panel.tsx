...
export function CalendarPanel({ tasks, onTaskClick }: CalendarPanelProps) {
  ...
  // Invalidate all task-related queries (dashboard + allStatus + instances)
  ...
  const handlePlanExecute = useCallback(
    async (startMinutes: number, endMinutes: number) => {
      // Use calendarCenterDate — plan mode always targets the center panel
  ...
  return (
    <div className="relative flex flex-col flex-1 min-h-0 md:border-r md:border-b md:rounded-br-[12px] md:shadow-[var(--shadow-card)]">
      {/* Calendar header */}
