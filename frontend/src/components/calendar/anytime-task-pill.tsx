...
export function AnytimeTaskPill({ task, onClick }: AnytimeTaskPillProps) {
  ...
  return (
    <button
      ref={setNodeRef}
      type="button"
      className={`text-[11px] truncate rounded-full px-2 py-0.5 hover:bg-accent/50 cursor-grab active:cursor-grabbing bg-card border border-border/40 max-w-[180px] flex-shrink-0 ${isDragging ? "opacity-30" : ""}`}
      style={{
